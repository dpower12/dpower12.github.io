...
function modifyCallbackPageLayout() {
    if (!window.location.pathname.includes("ExtraCallbacks")) {
        return;
    }

    var tableContent = "";
    $("article.md-content__inner .md-typeset__table").each(function(index) {
        var headerLink = $(this).find("td:eq(0)").text().toLowerCase();
        tableContent = tableContent + "<tr>" +
            "<td class=\"copyable\" style=\"text-align: right;\"><a href=\"#" + headerLink + "\">" + $(this).find("td:eq(0)").text() + "</a></td>" +
            "<td>" + $(this).find("td:eq(1)").html() + "</td>" +
            "<td>" + $(this).find("td:eq(2)").html() + "</td>" +
            "<td>" + $(this).find("td:eq(3)").html() + "</td></tr>";
    });
